...
}:
{
  ...
  networking = {
    ...
    wg-quick.interfaces."WARP" = {
      type = "amneziawg";
      autostart = false;   # ✅ Установить, но не запускать при старте
  ...
  services = {
    ...
    zapret = {
      enable = false;   # ✅ Установить, но не запускать при старте
      enableOnBoot = false;  # ❌ Не автозапуск
      udpSupport = true;
      udpPorts = [
        "50000:50099"
      ];
  ...
  # ✅ По умолчанию запускаем WARP
  # systemd.services."wg-quick@WARP".enable = true;
  # systemd.services."wg-quick@WARP".wantedBy = [ "multi-user.target" ];
