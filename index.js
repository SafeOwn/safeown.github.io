...
// --- НАЧАЛО ИЗМЕНЕНИЙ ---
// --- Определение компонента настроек ---
var settingsPlugin = {
    component: '4k_iptv_settings', // Уникальное имя компонента для настроек
    // Иконка для пункта меню настроек (шестеренка)
...
function settingsPage(object) {
    var html = $('<div></div>');
    var body = $('<div class="settings settings--iptv"></div>'); // Создаем контейнер для настроек
    var scroll = new Lampa.Scroll({mask: true, over: true});
    var save_button = $('<div class="settings-param selector"><div class="settings-param__name">'+langGet('save_settings')+'</div></div>');
    var reset_button = $('<div class="settings-param selector"><div class="settings-param__name">'+langGet('reset_settings')+'</div></div>');

    // --- Поля ввода ---
    // Название плейлиста
    var name_title = $('<div class="settings-param-title"><span>'+langGet('settings_list_name')+'</span></div>');
    var name_input = $(`<div class="settings-param selector">
        <div class="settings-param__name"></div>
        <div class="settings-param__value"></div>
        <input class="settings-param__input" type="text" placeholder="${langGet('settings_list_name_placeholder') || '4kTV'}">
    </div>`);
    var name_value = name_input.find('.settings-param__value');
    var name_field = name_input.find('.settings-param__input');

    // URL плейлиста
    var url_title = $('<div class="settings-param-title"><span>'+langGet('settings_list_url')+'</span></div>');
    var url_input = $(`<div class="settings-param selector">
        <div class="settings-param__name"></div>
        <div class="settings-param__value"></div>
        <input class="settings-param__input" type="text" placeholder="${langGet('settings_list_url_placeholder') || 'https://...'}">
    </div>`);
    var url_value = url_input.find('.settings-param__value');
    var url_field = url_input.find('.settings-param__input');

    // UID (только отображение)
    var uid_title = $('<div class="settings-param-title"><span>'+langGet('uid')+'</span></div>');
    var uid_static = $(`<div class="settings-param selector">
        <div class="settings-param__name">${langGet('uid')}</div>
        <div class="settings-param__value">${UID}</div>
    </div>`);

    // --- Инициализация значений ---
    var current_name = getSettings('list_name_0') || plugin.name; // Используем имя плагина по умолчанию
    var current_url = getSettings('list_url_0') || langGet('default_playlist');

    name_field.val(current_name);
    name_value.text(current_name || '---');
    url_field.val(current_url);
    url_value.text(current_url || '---');

    // --- Обработчики событий ---
    name_input.on('hover:enter', function(){
        Lampa.Input.edit({
            value: name_field.val(),
            free: true, // Разрешить любые символы
            title: langGet('settings_list_name')
        }, function (new_value){
            name_field.val(new_value);
            name_value.text(new_value || '---');
    ...
    save_button.on('hover:enter', function(){
        var new_name = name_field.val().trim();
        var new_url = url_field.val().trim();

        // Сохраняем в Storage
        setStorage('list_name_0', new_name || plugin.name); // Всегда сохраняем, даже если пусто
        setStorage('list_url_0', new_url); // Сохраняем URL

        // Обновляем отображение в пункте меню основного плагина (если он уже создан)
        var title = new_name || plugin.name;
        $('.js-' + plugin.component + '-menu0-title').text(title);

        // Проверяем URL и показываем/скрываем пункт меню основного плагина
        var menuEl = $('.js-' + plugin.component + '-menu0');
        var activity = lists[0] ? lists[0].activity : null;
        if (activity) {
             activity.title = title; // Обновляем заголовок активности
             if (/^https?:\/\/./i.test(new_url)) {
                activity.url = new_url;
                menuEl.show();
             } else {
                activity.url = '';
                menuEl.hide();
        ...
        Lampa.Notifications.show(langGet('settings_saved'), 3000);
        Lampa.Controller.toggle('content');
    });

    reset_button.on('hover:enter', function(){
         if (confirm(langGet('confirm_reset_settings'))) {
            // Сбрасываем настройки
            setStorage('list_name_0', plugin.name);
            setStorage('list_url_0', langGet('default_playlist'));

            // Обновляем поля
            name_field.val(plugin.name);
            name_value.text(plugin.name);
            url_field.val(langGet('default_playlist'));
            url_value.text(langGet('default_playlist'));

            // Обновляем отображение в пункте меню
            $('.js-' + plugin.component + '-menu0-title').text(plugin.name);
            var menuEl = $('.js-' + plugin.component + '-menu0');
            var activity = lists[0] ? lists[0].activity : null;
            if (activity) {
                 activity.title = plugin.name;
                 activity.url = langGet('default_playlist');
                 menuEl.show(); // URL по умолчанию корректный
                 // activity.url уже установлен в default_playlist в configurePlaylist
            }

            Lampa.Notifications.show(langGet('settings_reset'), 3000);
         }
         Lampa.Controller.toggle('content');
    });
    ...
    body.append(url_input);
    body.append(uid_title);
    body.append(uid_static);
    ...
    // --- Методы компонента страницы настроек ---
...
function pluginPage(object) {
    // ... (остальной код pluginPage без изменений) ...
...
// --- НАЧАЛО ИЗМЕНЕНИЙ ---
// --- Добавляем переводы для настроек ---
langAdd('settings_saved',
    {
...
langAdd('confirm_reset_settings',
    {
        ru: 'Вы уверены, что хотите сбросить настройки плагина?',
        uk: 'Ви впевнені, що хочете скинути налаштування плагіна?',
        be: 'Вы ўпэўнены, што жадаеце скінуць налады плагіна?',
        en: 'Are you sure you want to reset the plugin settings?',
        zh: '您确定要重置插件设置吗？'
    }
);
langAdd('settings_list_name_placeholder',
    {
        ru: 'Введите название',
        uk: 'Введіть назву',
        be: 'Увядзіце назву',
...
langAdd('settings_list_url_placeholder',
    {
        ru: 'Введите URL плейлиста',
        uk: 'Введіть URL списку відтворення',
        be: 'Увядзіце URL спіса прайгравання',
...
// --- НАЧАЛО ИЗМЕНЕНИЙ ---
// --- Удалены функции addSettings и configurePlaylist как они были ---
// --- КОНЕЦ ИЗМЕНЕНИЙ ---

// --- НАЧАЛО ИЗМЕНЕНИЙ ---
// --- Новая функция configurePlaylist для работы без SettingsApi ---
function configurePlaylist(i) {
    // Обрабатываем только первый плейлист (i=0)
    if (i > 0) return i + 1;

    var defName = plugin.name; // Используем имя из объекта plugin
    var activity = {
        id: i,
        url: '', // Изначально URL пустой
        title: defName, // Изначально заголовок по умолчанию
        groups: [],
    ...
    var storedName = getSettings('list_name_' + i); // Может быть пустым
    var storedUrl = getSettings('list_url_' + i); // Может быть пустым

    // --- Устанавливаем значения из Storage или по умолчанию ---
    var finalName = (storedName || defName); // Если имя не задано, используем дефолтное
    activity.title = finalName;
    ...
    var menuEl = $('<li class="menu__item selector js-' + plugin.component + '-menu' + i + '">'
                    ...
                    + '<div class="menu__text js-' + plugin.component + '-menu' + i + '-title">'
                        + encoder.text(finalName).html() // Отображаем имя из Storage или дефолтное
                    + '</div>'
                + '</li>')
    ...
    // --- Проверяем URL из Storage ---
    // Используем тот же паттерн, что и в оригинальной функции onChange
    if (/^https?:\/\/./i.test(storedUrl)) {
        activity.url = storedUrl;
        menuEl.show(); // Показываем пункт меню, если URL корректный
    }
    // Если URL пуст или некорректный, menuEl остается скрытым

    lists.push({activity: activity, menuEl: menuEl, groups: []});
    return !activity.url ? i + 1 : i; // Если URL нет, возвращаем i+1, иначе i
}
// --- КОНЕЦ ИЗМЕНЕНИЙ ---
...
Lampa.Component.add(settingsPlugin.component, settingsPage);
// --- КОНЕЦ ИЗМЕНЕНИЙ ---

// Готовим настройки (для квадратных иконок и т.д., которые не требуют SettingsApi)
// Lampa.SettingsApi.addComponent(plugin); // --- УДАЛЕНО ---
addSettings(
    'trigger',
    {
...
// for (var i=0; i <= lists.length; i++) i = configurePlaylist(i); // --- УДАЛЕНО ---
// --- НАЧАЛО ИЗМЕНЕНИЙ ---
// --- Вызываем configurePlaylist только для i=0 ---
configurePlaylist(0);
// --- КОНЕЦ ИЗМЕНЕНИЙ ---
...
// addSettings('title', {title: langGet('uid')}); // --- УДАЛЕНО ---
// addSettings('static', {title: UID, description: langGet('unique_id')}); // --- УДАЛЕНО ---
//~ Готовим настройки

// --- НАЧАЛО ИЗМЕНЕНИЙ ---
// --- Удален listener для Settings 'open' ---
// --- КОНЕЦ ИЗМЕНЕНИЙ ---
...
function pluginStart() {
    ...
    // --- НАЧАЛО ИЗМЕНЕНИЙ ---
    // --- Добавляем пункт меню для основного плагина ---
    for (var i=0; i < lists.length; i++) menu.append(lists[i].menuEl);

    // --- Добавляем пункт меню для настроек ---
    var settingsMenuEl = $('<li class="menu__item selector">'
                    + '<div class="menu__ico">' + settingsPlugin.icon + '</div>'
                    + '<div class="menu__text">' + encoder.text(settingsPlugin.name).html() + '</div>'
                + '</li>')
        .on('hover:enter', function(){
             // Создаем и запускаем активность для настроек
             var settingsActivity = {
                 component: settingsPlugin.component
                 // Можно добавить другие параметры активности, если нужно
             };
             Lampa.Activity.push(settingsActivity);
        });
